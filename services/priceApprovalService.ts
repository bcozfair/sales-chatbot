 ...
 *
 * **ไม่แตะร่างเดิมตรงนี้** — ร่างเดิมถูก *ทิ้งทั้งแถว* หลังใบใหม่ถูกสร้างสำเร็จเท่านั้น
 * (`replaces_request_id` ที่ `createDraft`) ไม่งั้นคนที่กดแก้แล้วปิดจอไป จะเหลือมือเปล่า
 *
 * ทิ้ง ไม่ใช่มาร์ก `cancelled` — "แก้แล้วส่งใหม่" คือการแก้ทับร่างเดิม สถานะ `cancelled`
 * เหลือไว้ให้ `cancelRequest` ซึ่งเป็นการที่ **มีคนกดยกเลิกจริง ๆ** (เจ้าของสั่ง 2026-09-21)
 */
