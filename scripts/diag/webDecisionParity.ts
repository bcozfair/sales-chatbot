 ...
 *
 * **ตอนนี้ว่างเปล่า และควรอยู่แบบนั้น** — ลิสต์นี้ไม่ใช่ที่ทิ้งขยะ
 *
 * ประวัติ: 2026-09-14 เคยมี `pin-4-person-phone` อยู่ในนี้ 1 วัน แล้ว **ถอดออกเพราะมันไม่ใช่บั๊ก** —
 * ระบบเลือก "คุณโยธิน นามบุรี" ถูกแล้ว ที่ผิดคือเฉลยในชุดข้อสอบต่างหาก (เคยเขียนว่า "คุณ โยธิน ปาทาน")
 * รายละเอียดหลักฐานอยู่ที่ scripts/evalCustomerSearch.ts เหนือเคสนั้น และ
 * docs/plan-web-quote-request.md §L.5
 *
 * บทเรียนที่แพงที่สุดของรอบนั้น: **"ด่านแดง" ไม่ได้แปลว่าโค้ดผิดเสมอไป — เฉลยผิดก็ทำให้ด่านแดงได้**
 * ก่อนจะเพิ่มชื่อลงลิสต์นี้ ต้องไล่ให้จบก่อนว่าเฉลยถูกจริงไหม ไม่งั้นจะกลบบั๊กที่ไม่มีอยู่จริง
 * แล้วเสียเวลาไปแก้ชั้นที่ไม่ได้พัง (รอบนั้นเกือบไปแก้ stripPhoneNumbers ทั้งที่เบอร์ในเคสนั้น
 * ไม่มีอยู่ใน DB เลยสักแถว)
 */
const KNOWN_INHERITED_WRONG = new Set<string>([]);
