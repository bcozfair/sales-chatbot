...
### L.5 หนี้ที่เจอระหว่างทาง — **ยังไม่ได้แก้**

`evalCustomerSearch --ai` รายงาน `wrong-auto-select : 1` ซึ่ง **ผิดกติกาที่ `CLAUDE.md` เขียนว่า
ต้องเป็น 0 ตลอด** · วัด 2026-09-14 บนโค้ดที่ยังไม่ได้แตะเส้นค้นหาเลย ⇒ **เป็นของเดิมที่มีอยู่บน
production วันนี้ ไม่ได้เกิดจากงานนี้**

เคส `pin-4-person-phone`: `"คุณโยธิน 06-3884-0005"` → ระบบเลือก *"คุณโยธิน นามบุรี"* (0.000057)
เฉลยคือ *"คุณ โยธิน ปาทาน"* อยู่อันดับ 2 (0.139) · ลูกค้าบุคคลชื่อซ้ำกัน และ **เบอร์โทรซึ่งเป็น
ตัวชี้ขาดตัวเดียวถูก `stripPhoneNumbers` ตัดทิ้งก่อนค้น** ⇒ แก้ที่ชั้นตัดสินใจไม่ได้
ต้องแก้ที่ชั้นหลักฐาน · ตอน AI ปิด เคสนี้ไม่ผิด

หน้าเว็บรับพฤติกรรมนี้มาด้วยเพราะใช้กฎเดียวกัน **แต่ผลเสียเบากว่าฝั่ง LINE**: บนเว็บมันคือค่า
preselect ใน dropdown ที่ยังเห็นตัวเลือกอื่นครบและเปลี่ยนได้ ส่วนบน LINE คือการออกร่างไปเลย

ด่านจึงขึ้นบัญชีเคสนี้ไว้ที่ `KNOWN_INHERITED_WRONG` — **ไม่ล้มเพราะมัน แต่เคสใหม่ที่ไม่อยู่ในบัญชี
ทำให้ล้มทันที**
