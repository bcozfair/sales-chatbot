...
**คำตอบของเจ้าของ (2026-09-23): "ตามที่แนะนำ"** — ทุกข้อเป็นตัวหนาในตารางข้างบน · ข้อ 9 ใช้ถ้อยคำที่แนะนำตรงตัว
(`NO_BOOK_MESSAGE` ใน `bookStore.ts`) · ข้อ 13: เจ้าของสั่งเฟส 3 เอง ช่วง 20:00–06:00

---

## 13. สถานะการลงมือ (2026-09-23)
...
| ด่าน | ก่อน | หลัง |
| --- | --- | --- |
| `pricingLab.ts` | 21/21 | 21/21 (ข้อ 5 อยู่ใน ROLLBACK แล้ว) |
| `pricingGolden.ts` | 64/64 | 64/64 |
| `pricingRoundtrip.ts` | 30/30 | 30/30 |
| `pricingImport.ts` | 26/26 | 16/16 + 10 ข้อย้ายไป `pricing-db` |
| `pricingModelEdit.ts` | 33/33 | 33/33 |
| `pricingDbRoundtrip.ts` (ใหม่) | — | **51/51** ทั้ง `--data` · `--from-json` · เล่มในฐาน · ข้อ 4 ตรวจราคา 17,787 รหัส |
| `diag:pricing-coverage` | 80.1% | 80.1% |
| API ผ่าน router จริง (สคริปต์ชั่วคราว ไม่ commit) | — | 24/24 รวม **สองคนกดบันทึกพร้อมกัน → 200 หนึ่ง · 409 หนึ่ง** · ไม่ส่ง `fingerprint` → 400 · ลายนิ้วมือ sha256 ของยุคไฟล์ → 409 |
| `diag:pb-ui` (หน้าจริง · ฐานชั่วคราว · Chromium `--no-sandbox` เฉพาะสำเนาในกล่อง) | ล้ม 27 | ล้ม 26 — **ขั้นอัปไฟล์ล้มเหมือนกันทั้งสองรุ่น** (ปัญหาเดิมของด่าน ยังไม่ได้ไล่) · ข้อที่ขึ้นกับชั้นเก็บผ่าน: การ์ด · รายการเล่ม · ปุ่มย้อน · ราคาคืนครบ · ไม่มี JS error |

**ต่างจากแบบใน §5–§8 (ตั้งใจ):**
- ตัวหาเล่มของด่านอยู่ที่ `scripts/pricebook/bookSource.ts` ไม่ใช่ `scripts/diag/pricingBookSource.ts` — CLI `pricebook:xlsx`/`pricebook:calc` ก็ใช้ตัวเดียวกัน
- แปลง 23505 เป็น `BookConflict` ด้วย "ตาราง `pricing_book_revisions` + ชื่อ constraint มีคำว่า parent" ไม่ใช่ชื่อตรงตัว — ตารางเงาของด่าน (`LIKE … INCLUDING ALL`) ได้ชื่อ `…_parent_id_key` · ด่านข้อ 7 จึงพิสูจน์การแปลงได้จริง
- `loadBookState(db = pool)` รับ executor ได้ — ให้ด่านข้อ 14 พิสูจน์ cache บนตารางชั่วคราว (แอปใช้ค่าเริ่มต้นเสมอ)
- ด่านข้อ 0 ถาม `pg_namespace` ตรง ๆ แทน `to_regclass()::text` ซึ่งไม่พิมพ์ชื่อ schema ของตารางที่มองเห็นอยู่ (รอบแรกด่านหยุดตัวเองก่อนเขียนเพราะข้อนี้ — ทำงานตามที่ออกแบบ)
- ชุดควบคุมข้อ 5 แปลงผ่าน `::jsonb` ของ Postgres จริง ไม่ใช่จำลองลำดับคีย์ใน JS
- `route /rollback` ที่ชนการบันทึกของคนอื่นตอบ 409 (ยุคไฟล์ไม่มีกรณีนี้)
