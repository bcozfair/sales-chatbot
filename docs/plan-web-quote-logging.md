...
`outcome` กับ `auto_customer_id` เป็น **คนละคำถาม อย่าอ่านแทนกัน** — `outcome` บอกรูปร่างของ
ผลค้น (เจอกี่ราย) ส่วน `auto_customer_id` บอกว่าชั่งคะแนนแล้วชี้ขาดได้หรือไม่
⇒ แถว `outcome='ambiguous'` ที่มี `auto_customer_id` คือเคสที่ชั้นตัดสินใจทำงาน (เพิ่ม 2026-09-14
พร้อม `plan-web-quote-request.md` §L) · แถวก่อนหน้านั้นไม่มีคีย์นี้เลย ใช้แยกยุคข้อมูลได้
...
### §5.1 ข้อจำกัดที่ต้องรู้ตอนอ่านตัวเลข — **แก้แล้ว 2026-09-14 บ่าย**

> **เดิม:** `webQuoteService.ts` เรียก `findContactCandidates` **เฉพาะตอนบริษัทเหลือตัวเดียว** ⇒
> `contact_candidates` จะเป็น `[]` ในเคสที่บริษัทกำกวม ซึ่งเป็นเคสที่เราสนใจที่สุดพอดี
> เขียนไว้ตอนนั้นว่า "ห้ามแก้จุดนั้นในเฟสนี้ เพราะมันคือหนึ่งในสิ่งที่งานแก้ parity จะเปลี่ยน"

งาน parity ทำแล้ว (ดู `docs/plan-web-quote-request.md` §L) — เงื่อนไขเปลี่ยนจาก "นับจำนวน"
เป็น "ผลของ `decideCustomerSelection`" ⇒ เคสที่ระบบชี้ขาดได้จะมี `contact_candidates` ให้ด้วย
ส่วนเคสที่ยังกำกวมจริง ๆ ยังเป็น `[]` เหมือนเดิมโดยตั้งใจ (ยังไม่รู้ว่าจะค้นผู้ติดต่อของบริษัทไหน)

⚠️ **ตัวเลข `chosen_rank` ก่อน/หลัง 2026-09-14 เทียบกันไม่ได้** — แถวก่อนหน้านั้นเกิดจากกติกา
"นับจำนวน" ส่วนแถวหลังจากนั้นเกิดจากกติกาชั่งคะแนน · เวลาอ่านผลให้ใส่เงื่อนไข
`created_at` หรือดูว่า `meta` มีคีย์ `auto_customer_id` ไหม (แถวเก่าไม่มี)
