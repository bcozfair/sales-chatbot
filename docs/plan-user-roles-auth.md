...
**ช่องค้นหา · ตัวกรองสิทธิ์ · แบ่งหน้า — เพิ่ม 2026-09-24** ตอนที่บัญชีจริงขึ้นเป็น **45 ใบ
และ 29 ใบเป็น `salesperson`** (หลังเปิดบัญชีพนักงานขายครบตาม `plan-role-permissions.md` §14)
รายชื่อที่เรียงยาวลงไปแถวเดียวจึงกลายเป็นการกวาดตาทั้งหน้า
* ประกอบจากของร่วมล้วน — `FilterBar`+`FilterSearch`+`FilterSelect` และ `Pagination`+`EmptyState`
  ของ `logs/ui.tsx` **ไม่มีคอมโพเนนต์ใหม่** (จำนวนต่อหน้าจึงเป็น 20/50/100/200 ของกลาง)
* **กรองในเครื่องทั้งหมด ไม่แตะ API** — `GET /api/admin/users` คืนครบทุกแถวอยู่แล้วตั้งแต่โหลดครั้งแรก
  ⇒ ไม่ต้อง debounce และไม่ต้องเพิ่มพารามิเตอร์ให้ endpoint
* ตัวเลือกใน dropdown มาจาก **แถวที่โหลดมาแล้ว** ไม่ใช่ `ROLE_ORDER` ทั้งชุด — role ที่ยังไม่มีใครถือ
  จะไม่โผล่เป็นตัวเลือกที่เลือกแล้วได้ตารางว่าง (`docs/design.md` ข้อ 3)
* ช่องค้นหาค้น **รหัสพนักงานขาย** ด้วย และรหัสขึ้นเป็นชิปใต้ชื่อในตาราง — สองอย่างนี้เป็นคู่กัน
  ถ้าค้นด้วยรหัสได้แต่ไม่แสดงรหัส คนใช้จะไม่มีอะไรบนจอยืนยันว่าทำไมแถวนั้นถึงขึ้นมา
  (คุณวิรุณ `THT0015` ถือสองรหัส 441 + 688 ⇒ ค้นเลขใดเลขหนึ่งก็เจอ)
* **เลขหน้าถูกหนีบตอนคำนวณ ไม่ใช่ `setState` ใน effect** — ลบบัญชีสุดท้ายของหน้าสุดท้ายแล้ว
  ต้องไม่ค้างอยู่หน้าว่าง และ `react-hooks/set-state-in-effect` ห้ามท่าหลังอยู่แล้ว
* ⚠️ **ที่ 390px ตารางยังเป็นของเดิม** (เลื่อนแนวนอนในกล่องตัวเอง · คอลัมน์ "จัดการ" ตกขอบ)
  เจ้าของเลือกไว้ 2026-09-24 ว่ายังไม่ทำ — การเปลี่ยนเป็น **การ์ดต่อแถว** แบบหน้าข้อมูลลูกค้า
  เป็นงานของตัวเอง ไม่ใช่ผลข้างเคียงของงานนี้
