...
| การจับคู่รหัสสินค้า (`findProduct` · `thaiSuffixVariant`) | `npm run diag:product-sequence` (อ่านอย่างเดียว ~3 วิ · ข้อที่ห้ามล้ม: **ชื่อรุ่นทุกตัวในฐานพิมพ์มาเป๊ะต้องได้ตัวเอง** — เคยพลาด 3 รุ่นที่มีคำไทยกลางชื่อ) · `npm run diag:product-match-line` — ยิงข้อความผ่าน `handleEvent` จริงทั้งทางขอใบและทางถามราคา (11 เคสจากข้อความจริงที่เคยผิด: "ดูดออก" · ผิดซีรีส์) · **commit แถวจริงแล้วลบ ต้องขอเจ้าของก่อน** · `diag:product-candidates` วัดแค่ `buildCandidateList` ไม่ครอบ findProduct |
