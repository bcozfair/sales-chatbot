...
| โมดูล "คิดราคาสินค้า" (`services/pricingLab/`) | `npm run diag:pricing` — รวม golden 35 เคส + roundtrip 26 เคส + ด่านของโมดูล · **เขียนฐานจริงแล้วลบทิ้งใน `finally`** · ต้องมี `pricebook/book.json` ก่อน (สร้างด้วย `prototypes/pricing/importer.ts` แล้วคัดลอกมา — ไม่มีไฟล์ = ด่านบอกวิธีสร้างแล้วหยุด ไม่ใช่ล้มแบบงง ๆ) |
...
รายการเต็มอยู่ใน `package.json` (วัด 2026-09-21: 62 รายการใน `scripts/diag/` — 57 `.ts` ·
