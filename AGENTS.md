...
| โมดูล "คิดราคาสินค้า" (`services/pricingLab/`) | `npm run diag:pricing` — รวม golden 62 เคส + roundtrip 26 เคส + ด่านของโมดูล · **เขียนฐานจริงแล้วลบทิ้งใน `finally`** · ต้องมี `pricebook/book.json` ก่อน (สร้างด้วย `npm run pricebook:import` ซึ่งเขียนลงที่นั้นให้เอง — ไม่มีไฟล์ = ด่านบอกวิธีสร้างแล้วหยุด ไม่ใช่ล้มแบบงง ๆ) |
