...
| ปุ่มนำเข้า/ส่งออกสมุดราคาบนหน้าแอดมิน | `npm run diag:pricing-import` (อยู่ใน `diag:pricing` แล้ว · ทำงานบนสำเนาชั่วคราว ไม่แตะสมุดจริง) **บวก `npm run diag:pb-ui`** ซึ่งเปิดหน้าจริงกดจริงที่ 1280/390 — ด่านหลังนี้ **เขียนทับ `pricebook/book.json` ของ cwd ที่เซิร์ฟเวอร์รันอยู่** (แล้วกดย้อนคืนตอนจบ) ⇒ รันกับเซิร์ฟเวอร์ที่เปิดจาก **worktree ของงาน** เท่านั้น · ข้อที่ห้ามล้มคือ "อัปไฟล์เดิมกลับ = ไม่มีช่องไหนเปลี่ยน" (ถ้าล้ม แค่กดสองปุ่มติดกันราคาก็ขยับ) และ "รุ่นที่ไม่ติ๊ก คงราคาเดิม" (จอสัญญาไว้ตรงนั้น) |
| โมดูล "คิดราคาสินค้า" (`services/pricingLab/`) | `npm run diag:pricing` — รวม golden 62 เคส + roundtrip 26 เคส + import 26 เคส + ด่านของโมดูล · **เขียนฐานจริงแล้วลบทิ้งใน `finally`** · ต้องมี `pricebook/book.json` ก่อน (สร้างด้วย `npm run pricebook:import` ซึ่งเขียนลงที่นั้นให้เอง — ไม่มีไฟล์ = ด่านบอกวิธีสร้างแล้วหยุด ไม่ใช่ล้มแบบงง ๆ) |
| ลอกตารางราคาเพิ่ม / แตะตัวอ่านรหัสสินค้า (`code.ts`) | `npm run diag:pricing-coverage` — ถามว่า **"รหัสจริงในฐานกี่ % ที่รุ่นของมันมีตารางราคาในสมุดแล้ว"** (วัด 2026-09-21: **80.1%** จาก 22,286 รหัส · พื้นตั้งไว้ 75%) · อ่านฐานแบบอ่านอย่างเดียว · ไม่มี `pricebook/book.json` = **ตอบไม่ได้ ไม่ใช่ตก** · ⚠️ ตัวเลข "อ่านครบทุกรหัสย่อย" ที่พิมพ์คู่กัน **ลดลงได้ตอนงานสำเร็จ** (รุ่นใหม่พารหัสย่อยที่ยังไม่ได้ตั้งค่าเข้ามาในตัวหาร) จึงไม่ใช่เกณฑ์ผ่าน/ตก |
