...
**event ที่ `deliveryContext.isRedelivery === true` ตอนนี้ถูกข้ามทุกตัว** (ตั้งแต่ 2026-09-22)
— **และนั่นถูกแค่ 5 ใน 7 ครั้ง** วัด 2026-09-23: ของที่ส่งซ้ำมี **สองกลุ่มที่คนละเรื่อง**
· **A (5)** รอบแรกมาถึงและเราตอบไปแล้ว ⇒ ทิ้งถูก · **B (2)** **รอบแรกไม่เคยมาถึงแอปเลย**
⇒ ทิ้งแล้วคำสั่งของเซลส์หายเงียบ (เกิดจริง: ใบ `4ae3ef39` กดยืนยันแล้วค้างเป็นร่าง)
⚠️ **ข้อสรุปเดิมที่ว่า "token หมดอายุแล้วทุกตัว" ผิด** — ทดลองจริง 2026-09-23 token ที่ยัง
ไม่ถูกใช้ตอบได้ที่ **10 / 65 / 120 วินาที** (HTTP 200 ทั้งสามครั้ง) สาเหตุจริงของ 400
`Invalid reply token` คือ **token ถูกใช้ไปแล้ว** ⇒ กลุ่ม B ตอบได้ แต่เราเลือกทิ้งเอง
· **ห้ามแก้ด้วยการยืนยันใบให้เงียบ ๆ** (เจ้าของสั่ง 2026-09-23 — เซลส์จะไม่รู้ว่าร่างถูก
ยืนยันไปแล้วจริง) · สวิตช์นี้ **เปิดเองที่ LINE Console เมื่อ ~2026-09-16** ก่อนหน้านั้น
เงียบมา 3 เดือน และ **การส่งซ้ำคืออาการ ไม่ใช่สาเหตุ** ปิดไปก็แค่ไม่เหลือร่องรอย
⇒ ตัวเลข หลักฐาน และสิ่งที่ยังไม่รู้ทั้งหมดอยู่ใน **`docs/line-webhook-redelivery.md`**
