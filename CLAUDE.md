...
**นาฬิกาคือข้อจำกัดที่ออกแบบทุกอย่างในเส้นนี้** — เราตั้งงบว่า `replyToken` อายุ 60 วินาที
นับจากรับ webhook (LINE ไม่ประกาศค่าจริงและเปลี่ยนได้โดยไม่แจ้ง · วัดเองได้ ≥120 วิ ดู
`docs/line-webhook-redelivery.md` — **ตัวเลขนั้นห้ามเอามายืดงบ**)
...
**`timeout` ของ OpenAI SDK กันได้แค่ครึ่งเดียว — มันนับถึงแค่ตอนได้ response header**
(วัดเอง 2026-09-23 · openai 6.38.0 · Node 22): ปลายทางที่ตอบ header เร็วแล้วพ่น body ช้า
**วิ่งได้ไม่จำกัดเวลา** ⇒ เกิดจริง 1 call กิน **93 วินาทีแล้วสำเร็จ** (ไม่ใช่ error) งบ 48 วิหมดก่อน
เซลส์ไม่ได้คำตอบเลย · แบบนี้เกิด **3 ครั้งใน 44 วัน จาก 6,644 งาน** (p95 ปกติ 6.1 วิ)
⇒ `createChatCompletion` บังคับ `AbortSignal.timeout(LLM_HARD_CAP_MS)` ทุกครั้ง ซึ่งครอบช่วง
โหลด body ด้วย **ห้ามถอดออกแล้วหวังพึ่ง `timeout` ตัวเดียว** · ตั้ง 20 วิ (ไม่ใช่ 40) เพื่อให้
เหลืองบ ~28 วิ ไว้ลองใหม่ — quoteExtraction ข้าม attempt ถัดไปเมื่องบเหลือ < 8 วิ
⚠️ `llm_ms=0 llm_calls=0 own_ms=48000` ใน `api_logs` **ไม่ได้แปลว่าไม่ได้เรียก LLM** แต่แปลว่า
ตอนพิมพ์บรรทัดสรุป call ยังไม่กลับมา — เคยทำให้ไล่ผิดทางมาแล้ว
· นับครั้งที่เพดานทำงาน: `docker logs primus-chatbot-app-1 | grep -c '\[llm\] ⏱️'`
· gate: `npm run diag:llm-timeout` (จำลองด้วยเซิร์ฟเวอร์ในเครื่อง ไม่ยิงเน็ตจริง ~25 วิ)

**event ที่ `deliveryContext.isRedelivery === true` ต้องถามก่อนทิ้ง ไม่ใช่ทิ้งทุกตัว**
(ตั้งแต่ 2026-09-23) — ของที่ส่งซ้ำมี **สองกลุ่มที่คนละเรื่อง** วัด 2026-09-23: **A (5 ใน 7)**
รอบแรกมาถึงและตอบไปแล้ว ⇒ ทิ้งถูก · **B (2 ใน 7)** **รอบแรกไม่เคยมาถึงแอปเลย** ⇒ ทิ้งแล้ว
คำสั่งเซลส์หายเงียบ (เกิดจริง: ใบ `4ae3ef39` กดยืนยันแล้วค้างเป็นร่าง) ⇒ ตัดสินจากสถานะจริงใน
ตาราง **`webhook_events`** (1 แถว = 1 `webhookEventId` · เก็บถาวร) ไม่ใช่จากธงเพียว ๆ
⚠️ **ห้ามเพิ่มทางออก "ทำงานให้เลย"** — เจ้าของสั่ง 2026-09-23 ว่าการยืนยันใบสำเร็จโดยเซลส์
ไม่รู้ตัวแย่กว่าไม่ทำอะไร ⇒ `services/redeliveryPolicy.ts` มีแค่ `skip` กับ `warn` และด่านไล่ทุก
ชุดค่าเพื่อพิสูจน์ว่าไม่มีทางที่สาม · `db/webhookEventsRepo.ts` **ห้าม throw** (DB ล่ม = ตกกลับไป
พฤติกรรมเดิม ไม่ใช่ทำ webhook พังทั้งเส้น)
⚠️ **ข้อสรุปเดิมที่ว่า "token หมดอายุแล้วทุกตัว" ผิด** — ทดลองจริง 2026-09-23 token ที่ยังไม่ถูกใช้
ตอบได้ที่ **10 / 65 / 120 วินาที** (HTTP 200 ทั้งสามครั้ง) สาเหตุจริงของ 400 คือ **token ถูกใช้ไปแล้ว**
· สวิตช์ redelivery **เปิดเองที่ LINE Console เมื่อ ~2026-09-16** ก่อนหน้านั้นเงียบมา 3 เดือน และ
**การส่งซ้ำคืออาการ ไม่ใช่สาเหตุ** ปิดไปก็แค่ไม่เหลือร่องรอย (เจ้าของยืนยันเปิดไว้ 2026-09-23)
· `REPLY_TOKEN_TTL_MS = 60_000` เป็นงบที่เลือกให้ปลอดภัยไว้ก่อน **ห้ามยืดเพราะวัดได้ 120 วิ**
· gate: `npm run diag:redelivery` · ตัวเลขและหลักฐาน: **`docs/line-webhook-redelivery.md`**
...
เทียบ prompt กับ golden file ทีละตัวอักษร แก้ช่องว่างเมื่อไหร่ด่านล้มทันที · ส่วน Flex ที่ตอบกลับ
**ไม่มี golden แล้ว** (ตั้งแต่ 2026-09-25) — ด่านรันเคสบน "โค้ดก่อนแก้" กับ "โค้ดปัจจุบัน" ต่อกันบนฐานเดียวกัน
แล้วเทียบกันเอง เพราะ golden ของผลลัพธ์เน่าตามสต็อก/ราคาจริงจนล้มเองทั้งที่โค้ดไม่เปลี่ยน ⇒
**ห้ามเพิ่มด่านที่เทียบผลซึ่งขึ้นกับข้อมูลในฐานกับไฟล์ที่บันทึกไว้** ให้เทียบกับโค้ดก่อนแก้ หรือตรวจเฉพาะสิ่งที่ต้องจริงเสมอ
...
- **`customer_details.payment_terms` ไม่ได้แปลว่า "เครดิตของลูกค้ารายนี้" เสมอไป** (ตั้งแต่
  ...
  · ตั้งแต่ 2026-09-25 หน้าเว็บ **ตั้งค่าทับเป็น `Cash` ให้เอง** เมื่อลูกค้าติดด่านเครดิต (`credit_hold`
  ในก้อน `/party`) — เป็นค่าทับธรรมดาทุกประการ และด่านเครดิต **ยังบล็อกเหมือนเดิม** (เจ้าของเลือก)
  · gate: `npm run diag:web-credit-dormant`
...
| ค้นหา/จับคู่สินค้า/ลูกค้า | `services/productService.ts` (ท้าย `findProduct` มีด่านลำดับของรหัส กันผิดซีรีส์) · `services/thaiSuffixVariant.ts` (รุ่นที่ต่างกันแค่คำไทยท้ายชื่อ เช่น "ดูดออก") · `services/customerService.ts` |
...
- **`docs/changes-2026-09-24-meeting.md`** — สรุปการเปลี่ยนแปลงหน้าขอใบเสนอราคาตามความต้องการใหม่หลังประชุม 24/9/2026 (ข้อ 1–14 · รวมแถว "ส่วนลดเดิม" · ข้อมูลลูกค้าชุดเดียวเมื่อแยกใบ PM/THT · ป้าย "ค่าเดิม" ของเครดิต · ลูกค้าเครดิตไม่มีบิลเกิน 12 เดือน → Cash อัตโนมัติ · ข้อ 11–13 คำเตือนกฎแบบ A แยกตาม role · ปุ่ม "ขออนุมัติ" · ข้อ 14 ช่อง "Source" → `quotations.source_id` → คอลัมน์ K ของไฟล์ Odoo)
...
- **`docs/plan-pricing-subcodes.md`** — พิมพ์รหัสสินค้าสั่งทำแล้วได้ราคา + ตารางรหัสย่อยที่แอดมินตั้งราคา/กฎเองได้ (เฟส A–C ทำแล้ว · **อยู่ในแอดมินจริงแล้วเป็นโมดูลที่ถอดออกได้** — ดู `services/pricingLab/README.md` · สมุดราคาครอบ 14 รุ่น / 11 ชีต = 80.1% ของรหัสจริง · เฟส D ยังเป็นแบบที่เสนอ)
  ...
  **ตั้งแต่ 2026-09-23 แก้ราคา+กฎทีละรุ่นจากหน้าจอได้ด้วย** (`GET/PUT /api/admin/pricebook/model/:code` · `services/pricingLab/modelEditor.ts`) — ทางเดียวกับ Excel ทุกประการ ต่างแค่ความสะดวก
  **ตั้งแต่ 2026-09-23 แก้ราคาอยู่เมนู "สมุดราคา" แยกสิทธิ์ `page.pricebook`** (เมนูคิดราคาเปลี่ยนชื่อเป็น "คำนวณราคา" 2026-09-24 · สองหน้ามีปุ่มไปหากัน โผล่ตามสิทธิ์ของอีกหน้า) — หน้าคิดราคา (`page.pricing`) มีแค่ `GET /overview` + `POST /quote` และ `diag:role-permissions` บังคับไว้ · ชื่อ `BH-01,02` บนจอเป็นแค่ชื่อที่แสดง รหัสในฐานยังเป็น `BH-01`
  **ตั้งแต่ 2026-09-24 "1 ชีต Excel = 1 หน้า"** — ตารางเป็นแถวละชีต · ชีตที่ผ่าน `excelReady()` (วันนี้ TS-01+TS-01-0) เปิดเป็นตารางหน้าตาแบบชีต แก้ได้ทั้งตารางราคาตั้งสองแกน บันทึกทั้งชีตครั้งเดียวที่ `PUT /api/admin/pricebook/sheet/:sheet` · ชีตอื่นยังเป็นหน้าแก้ทีละรุ่นจนกว่าจะย้ายทีละชีต · ของรอบตารางในไฟล์: **ค่ามาตรฐานเมื่อรหัสไม่ระบุ (เกลียว `axisDefaults` · สาย `axisDefaultsBy`) มีผลกับราคา และยึดแคตตาล็อก ไม่ใช่คอลัมน์ G** (เจ้าของเคาะ) · ตัวหนังสือแดง/คอลัมน์เหลือง = `layout` แสดงผลอย่างเดียว · เล่มเก่าเติมด้วย `importer.ts --extras-only` = `DEPLOY.md` 4.11ข · หน้าคิดราคาแยก "รหัสบอกไม่ครบ" (`Violation.missing`) ออกจาก "ไม่รับผลิต"
...
- **`docs/pricing-code-ts-01.md`** — รูปแบบรหัส TS_-01 / TS_-01-0 จากแคตตาล็อกที่เจ้าของส่งมา (2026-09-24 · **ภาพชนะคอลัมน์ G ของชีต**) · ตัวอักษรหลัง M = ตารางรหัสย่อย ไม่ใช่โค้ด · อ่านไม่ครบ **ห้ามเติมสายตั้งต้นทับ** · ราคาสาย TS 160 (2026-09-25) **อยู่ในฐานเท่านั้น ไม่มีใน Excel** · `xNN` เกิน 5 mm คิดตามอัตรา Excel (r7 · เพิ่มกฎให้เล่มเดิมด้วย `importer.ts --new-rules`) · **ราคายึด Excel · Odoo ใช้วิเคราะห์เท่านั้น** (เจ้าของ 2026-09-25) · §6 = แคตตาล็อกชุดเต็ม `backup/Catalogue/` + ความหมาย `-S###` · ท้ายรหัส TS_-08/10 = รหัสย่อยแบบ **"เปิดกฎบวกเพิ่มของรุ่น"** (`option` · ราคาอยู่ที่กฎเดิม ห้ามก๊อปเป็น `flat`) · แถวรหัสย่อย **ค่าว่าง = "ยังไม่มีราคา" ไม่ใช่ 0** (หัว S/E/SS/SB · เกลียวมิล) · ค่าสายเกินมาตรฐาน **นับเฉพาะเมตรเต็ม** ทุกรุ่น (`round: 'floor'` · เจ้าของ 2026-09-25)
