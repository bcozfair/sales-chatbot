...
- **`docs/plan-pricing-subcodes.md`** — พิมพ์รหัสสินค้าสั่งทำแล้วได้ราคา + ตารางรหัสย่อยที่แอดมินตั้งราคา/กฎเองได้ (เฟส A–C ทำแล้ว · **อยู่ในแอดมินจริงแล้วเป็นโมดูลที่ถอดออกได้** — ดู `services/pricingLab/README.md` · สมุดราคาครอบ 14 รุ่น / 11 ชีต = 80.1% ของรหัสจริง · เฟส D ยังเป็นแบบที่เสนอ)
  ...
  **ตั้งแต่ 2026-09-24 "1 ชีต Excel = 1 หน้า"** — ตารางเป็นแถวละชีต · ชีตที่ผ่าน `excelReady()` (วันนี้ TS-01+TS-01-0) เปิดเป็นตารางหน้าตาแบบชีต แก้ได้ทั้งตารางราคาตั้งสองแกน บันทึกทั้งชีตครั้งเดียวที่ `PUT /api/admin/pricebook/sheet/:sheet` · ชีตอื่นยังเป็นหน้าแก้ทีละรุ่นจนกว่าจะย้ายทีละชีต · ข้อความ/ไฮไลต์รอบตารางแบบในไฟล์อยู่ที่ `PriceModel.layout` (แสดงผลอย่างเดียว engine ไม่อ่าน · เล่มเก่าเติมด้วย `importer.ts --layout-only` = `DEPLOY.md` 4.11ข)
