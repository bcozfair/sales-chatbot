...
- **`docs/plan-pricing-subcodes.md`** — พิมพ์รหัสสินค้าสั่งทำแล้วได้ราคา + ตารางรหัสย่อยที่แอดมินตั้งราคา/กฎเองได้ (เฟส A–C ทำแล้ว · **อยู่ในแอดมินจริงแล้วเป็นโมดูลที่ถอดออกได้** — ดู `services/pricingLab/README.md` · สมุดราคาครอบ 14 รุ่น / 11 ชีต = 80.1% ของรหัสจริง · เฟส D ยังเป็นแบบที่เสนอ)
  ...
  **ตั้งแต่ 2026-09-24 "1 ชีต Excel = 1 หน้า"** — ตารางเป็นแถวละชีต · ชีตที่ผ่าน `excelReady()` (วันนี้ TS-01+TS-01-0) เปิดเป็นตารางหน้าตาแบบชีต แก้ได้ทั้งตารางราคาตั้งสองแกน บันทึกทั้งชีตครั้งเดียวที่ `PUT /api/admin/pricebook/sheet/:sheet` · ชีตอื่นยังเป็นหน้าแก้ทีละรุ่นจนกว่าจะย้ายทีละชีต · ของรอบตารางในไฟล์: **"ชนิดสาย รุ่นเริ่มต้น" = `axisDefaultsBy` มีผลกับราคา** (เจ้าของยืนยัน) · ตัวหนังสือแดง/คอลัมน์เหลือง = `layout` แสดงผลอย่างเดียว · เล่มเก่าเติมด้วย `importer.ts --extras-only` = `DEPLOY.md` 4.11ข · หน้าคิดราคาแยก "รหัสบอกไม่ครบ" (`Violation.missing`) ออกจาก "ไม่รับผลิต"
...
- **`docs/pricing-code-ts-01.md`** — รูปแบบรหัส TS_-01 จากแคตตาล็อกที่เจ้าของส่งมา (2026-09-24) · เทียบกับตัวอ่านรหัส · ตัวอักษรหลัง M = ชนิดสาย/Ground **ห้ามเติมสายตั้งต้นทับ**
