...
LINE ส่งซ้ำเมื่อรอบแรกไม่ได้ 200 (tunnel หลุด / แอปล่มตอน deploy) โดยพก **replyToken เดิม** มาด้วย
วัด 2026-09-21→22: **8/8 ครั้งเป็นแบบนี้ และตัวที่มาเร็วที่สุดยังอายุ 62.7 วิ** = เกิน TTL ไปแล้วทุกตัว
⇒ ตอบไม่ได้แน่นอน การปล่อยเข้าคิวคือเสีย LLM + สล็อตคิวของคนที่ยังตอบทัน แล้วจบด้วย 400
`Invalid reply token` สองครั้ง (ครั้งที่สองคือ fallback ในตัว catch-all ซึ่งใช้ token ตัวเดิม
จึงพังคู่กันเสมอ — นี่คือที่มาของ error ที่ขึ้นเป็นคู่ในหน้าบันทึกระบบ)
· และมันกัน **งานซ้ำ** ด้วย: เคยเจอ `action=confirm` ของใบที่ยืนยันไปแล้วถูกส่งซ้ำมาให้ทำอีกรอบ
· อัตราที่วัดได้ **8 จาก 128 webhook (6.3%) ใน 16 ชม.** ⇒ ประมาณ 6% ของข้อความที่เซลส์พิมพ์
**ไม่เคยมาถึงแอปเลยในรอบแรก** ซึ่งเป็นปัญหาของทางเข้า (Cloudflare Tunnel) ไม่ใช่ของโค้ด
การข้าม redelivery **ไม่ได้ทำให้เคสพวกนี้ได้คำตอบ** — มันแค่หยุดเผางบและหยุด error ปลอม
