-- ผู้ติดต่อที่แอดมินเพิ่มเอง — ตาราง local_contacts + Arm 3 ของ customers_data_build + audit
-- เฟส I ก้อน I1 ของ docs/plan-local-contacts.md §3.1–§3.3 · แบบเต็มอยู่ที่ docs/plan-web-quote-request.md §5
--
-- ปัญหา: แอดมินเพิ่มผู้ติดต่อใต้บริษัทที่มีอยู่แล้วไม่ได้เลย ต้องรอให้มีคนไปคีย์ใน Odoo แล้ว sync กลับมา
-- และเขียนลง customers ตรง ๆ ไม่ได้เพราะ sync ทับทุกรอบ (หายเงียบ)
--
-- ทางแก้: ตารางของตัวเองที่ sync ไม่แตะ แล้ว "ต่อท่อ" เข้า customers_data_build ซึ่งเป็นจุดเดียว
-- ที่ทั้งระบบอ่านลูกค้า/ผู้ติดต่อ ⇒ คนที่เพิ่งเพิ่มโผล่ครบทุกที่ (แชท · LIFF · หน้าเว็บ · reverse lookup)
...
--    ข้อมูลต้นทางไม่ขยับ แล้วดูเหมือนรันแล้วแต่ตารางจริงยังเป็นของเดิม (CLAUDE.md เตือนไว้)
--      npx tsx -e "import('./scripts/sync/refreshCustomerDirectory.js').then(m=>m.refreshCustomerDataView({force:true}).then(console.log))"
--
-- ⚠️ ตารางว่าง ⇒ local_rows ว่าง ⇒ all_rows = base ทุกแถวทุกคอลัมน์ · พิสูจน์ด้วย md5 ของ view
--    ทั้งก้อนก่อน–หลัง (82,721 แถว · md5 aee27a082e24539af7a2950f9608bf59 · วัด 2026-09-18 บนฐาน dev)
--    และเป็นด่านข้อ 1 ของ npm run diag:local-contacts
...
-- (เทียบแบบ btrim ให้ตรงกับเกณฑ์ dedupe ของ local_taken ข้างล่างเป๊ะ)
CREATE UNIQUE INDEX IF NOT EXISTS idx_local_contacts_company_name
  ON public.local_contacts (company_id, btrim(contact_name));

-- ─────────────────────────────────────────────────────────────────────────────
--  2. audit — "ระบบเป็นคนติ๊ก" ต้องมีหลักฐานว่า actor เป็นระบบไม่ใช่คน
--
--  เติมตารางใหม่เข้าชุด trigger ของ 2026-09-03_04_audit_logs.sql (ห้ามแก้ไฟล์เก่า จึงทำที่นี่
--  ด้วยรูปเดียวกันเป๊ะ: 3 ตัวต่อตาราง เพราะ transition table รวม event ไม่ได้)
--  audit_stmt กลืน error ของตัวเองอยู่แล้ว ⇒ audit หายดีกว่าทำให้คนบันทึกไม่ได้
-- ─────────────────────────────────────────────────────────────────────────────
DO $mk$
BEGIN
  -- ฐานที่ยังไม่ได้รัน migration ของ audit ⇒ ข้ามไป ไม่ทำให้ทั้งไฟล์ล้ม
  IF to_regproc('public.audit_stmt') IS NULL THEN
    RAISE NOTICE '[audit] ข้าม local_contacts — ยังไม่มี public.audit_stmt บนฐานนี้';
    RETURN;
  END IF;

  DROP TRIGGER IF EXISTS trg_audit_ins ON public.local_contacts;
  DROP TRIGGER IF EXISTS trg_audit_upd ON public.local_contacts;
  DROP TRIGGER IF EXISTS trg_audit_del ON public.local_contacts;

  CREATE TRIGGER trg_audit_ins AFTER INSERT ON public.local_contacts
    REFERENCING NEW TABLE AS audit_new
    FOR EACH STATEMENT EXECUTE FUNCTION public.audit_stmt('local_contact', 'contact_name', 'contact_id');
  CREATE TRIGGER trg_audit_upd AFTER UPDATE ON public.local_contacts
    REFERENCING OLD TABLE AS audit_old NEW TABLE AS audit_new
    FOR EACH STATEMENT EXECUTE FUNCTION public.audit_stmt('local_contact', 'contact_name', 'contact_id');
  CREATE TRIGGER trg_audit_del AFTER DELETE ON public.local_contacts
...
-- ─────────────────────────────────────────────────────────────────────────────
--  3. Arm 3 — ต่อแถว local เข้า customers_data_build
--
--  ⚠️ ท่อน CREATE OR REPLACE VIEW ข้างล่างคัดมาจาก migrations/schema.sql ทั้งดุ้นด้วย sed
--     ไม่ได้พิมพ์ใหม่ · ที่เปลี่ยนมีสองจุดเท่านั้น:
--       (ก) เพิ่ม CTE 4 ตัว (local_anchor · local_team · local_taken · local_rows) + all_rows
--       (ข) SELECT ท้าย เปลี่ยน FROM base b → FROM all_rows b
--     comp / own_last / own_credit / ent_keys **ยังอ่าน base เหมือนเดิม** ⇒ แถว local
--     ไม่มีทางขยับค่าระดับบริษัทหรือคำตอบของด่านเครดิตได้เลยแม้แต่บิตเดียว
-- ─────────────────────────────────────────────────────────────────────────────
