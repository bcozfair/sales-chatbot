...
#### 4.11ข — เติม "หน้าตาในไฟล์ราคา" ลงเล่มที่มีอยู่แล้ว (ครั้งเดียว · ต้องได้คำสั่งเจ้าของ · ไม่แตะราคา)

ตั้งแต่ 2026-09-24 ตัวนำเข้าอ่านสามอย่างรอบตารางราคาตั้งด้วย (คอลัมน์ "ชนิดสาย รุ่นเริ่มต้น" · ตัวหนังสือแดงใต้คอลัมน์ ·
คอลัมน์พื้นเหลือง — วันนี้เฉพาะชีต `TS-01+TS-01-0`) แต่เล่มในฐานบูตก่อนหน้านั้น ⇒ เติมด้วย `--layout-only`
ซึ่งแตะเฉพาะช่อง `layout` (**ห้ามใช้ `--replace-all` แทน** — ทับราคาที่แอดมินแก้จากจอ)

```bash
# ดูรายงานก่อน (ไม่เขียน) — ต้องเห็น TSK-01 และ TSK-01-0
docker compose run --rm --no-deps -v "$PWD/backup:/seed:ro" app \
  npx tsx scripts/pricebook/importer.ts --data /seed --layout-only
