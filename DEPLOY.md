...
**ชนิดสายรุ่นเริ่มต้นตาม TYPE** (`axisDefaultsBy` — มีผลกับราคา: รหัสที่ไม่บอกชนิดสายคิดได้) · ตัวหนังสือแดงใต้คอลัมน์ ·
คอลัมน์พื้นเหลือง · ชื่อกฎค่าสาย ⇒ เล่มในฐานที่บูตก่อนหน้านั้นเติมด้วย `--extras-only` (ชื่อเดิม `--layout-only` ยังใช้ได้)
ซึ่งไม่แตะตัวเลขราคาสักช่อง (**ห้ามใช้ `--replace-all` แทน** — ทับราคาที่แอดมินแก้จากจอ)
...
ส่วนชนิดสายรุ่นเริ่มต้น **ยังไม่ได้เขียน** — รันคำสั่งข้างบนซ้ำ (รายงานจะเหลือแค่ของที่ยังไม่มี) · ถ้า image ที่ deploy อยู่ยังไม่มี
