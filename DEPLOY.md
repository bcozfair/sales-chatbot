...
### ก่อนเริ่ม — เลือกเวลา deploy

**ช่วงที่บอทเงียบจริงคือช่วงสลับกล่องใน ขั้น 5 เท่านั้น** (ปกติไม่ถึง 10 วิ: หยุดตัวเก่า + boot ตัวใหม่
3.3 วิ) — ข้อความ LINE ที่เข้ามาช่วงนั้น **หายถาวร** เซลส์จะไม่ได้คำตอบเลย LINE ส่งซ้ำมาทีหลังก็จริง
แต่ `replyToken` หมดอายุแล้ว และเราห้ามใช้ push ⇒ แอปข้ามมันตามกติกา `isRedelivery` ใน `CLAUDE.md`
ถ้าตัวเก่ามีงานค้างตอนสั่งหยุด มันจะรอให้ตอบจบก่อน ⇒ ช่วงเงียบยืดได้ถึง ~50 วิ

จำนวน webhook เฉลี่ยต่อชั่วโมง (วัดจาก `api_logs` 14 วัน ถึง 2026-09-23):

| ช่วง | ต่อชั่วโมง | |
| --- | --- | --- |
| 20:00–06:00 | ≈ 0 | ✅ ดีที่สุด |
| 18:00–20:00 | 1–4 | ✅ |
| 12:00–13:00 | 15.6 | ใช้ได้ถ้าต้องขึ้นกลางวัน |
| 09:00–11:00 · 14:00–17:00 | 28–47 | ❌ เลี่ยง (พีค 11:00 = 47) |

วันอาทิตย์ 6 ข้อความทั้งวัน · เสาร์ 116 · วันธรรมดา 760–1,058
**ไม่ต้องประกาศปิดปรับปรุงสำหรับ deploy ปกติ** — ประกาศ "ปิด 5 นาที" ทำให้คนหยุดใช้นานกว่าที่บอท
เงียบจริงหลายเท่า · ถ้ามี migration ที่ทำให้ค้นหาลูกค้าพังเป็นนาที (ดูกฎใน ขั้น 4) ค่อยแจ้งเซลส์ล่วงหน้า
...
#### 4.11ข — เติม "ของรอบตาราง" จากไฟล์ราคาลงเล่มที่มีอยู่แล้ว (ต้องได้คำสั่งเจ้าของ · ไม่แตะตัวเลขราคา)

ตั้งแต่ 2026-09-24 ตัวนำเข้าอ่านของรอบตารางราคาตั้งด้วย (วันนี้เฉพาะชีต `TS-01+TS-01-0`):
**ค่ามาตรฐานเมื่อรหัสไม่ระบุ ตามแคตตาล็อก** (มีผลกับราคา — `docs/pricing-code-ts-01.md`): เกลียว TS_-01 = 1/4” ·
TS_-01-0 = M5 (`axisDefaults`) · ชนิดสาย = สแตนเลสถักทุก TYPE (`axisDefaultsBy` · **ไม่ใช่** คอลัมน์ G ของชีต —
เจ้าของเคาะให้ยึดแคตตาล็อก) · ตัวหนังสือแดงใต้คอลัมน์ · คอลัมน์พื้นเหลือง · ชื่อกฎค่าสาย ⇒ เล่มในฐานที่บูต
ก่อนหน้านั้นเติมด้วย `--extras-only` (ชื่อเดิม `--layout-only` ยังใช้ได้) ซึ่งไม่แตะตัวเลขราคาสักช่อง
(**ห้ามใช้ `--replace-all` แทน** — ทับราคาที่แอดมินแก้จากจอ)

**คู่กับตารางรหัสย่อยจากแคตตาล็อก** — ตัวอักษรหลัง M (ชนิดสาย T/P/F/C/TS + Ground U) และแกน 6 ของ TS_-01
อยู่ในตาราง `pricing_subcodes` (แอดมินแก้จากหน้าสมุดราคาได้) · ค่าตั้งต้นอยู่ที่ `scripts/pricebook/catalog-subcodes.json`
· เขียนแถวที่ยังไม่มีเท่านั้น แถวที่มีคนแก้แล้วข้าม — ทำ **สองคำสั่งนี้คู่กันเสมอ** ไม่งั้นรหัสที่บอกสายเองจะขึ้น "บอกไม่ครบ"
```bash
docker compose run --rm --no-deps app npx tsx scripts/pricebook/seedCatalogSubcodes.ts                       # รายงาน
docker compose run --rm --no-deps app npx tsx scripts/pricebook/seedCatalogSubcodes.ts --apply --by <username>
```

```bash
# ดูรายงานก่อน (ไม่เขียน) — ต้องเห็น TSK-01 และ TSK-01-0
docker compose run --rm --no-deps -v "$PWD/backup:/seed:ro" app \
  npx tsx scripts/pricebook/importer.ts --data /seed --extras-only
# เขียนจริง = การบันทึก 1 ครั้ง (ย้อนได้จากปุ่มย้อนเล่ม)
docker compose run --rm --no-deps -v "$PWD/backup:/seed:ro" app \
  npx tsx scripts/pricebook/importer.ts --data /seed --extras-only --apply --by <username>
```
**ฐานของ PMSV:** ส่วนหน้าตาทำแล้ว 2026-09-24 (การบันทึกครั้งที่ 4 · สำรองที่ `backup/pricing-before-layout-2026-09-24-0909.dump`)
ส่วนค่ามาตรฐานตามแคตตาล็อก + ตารางรหัสย่อย **เขียนแล้ว 2026-09-25** (เจ้าของสั่ง "เขียนได้" · การบันทึกครั้งที่ 5 + 11 แถว ·
สำรองที่ `backup/pricing-before-catalog-2026-09-25-0856.dump` · ตัวเลขราคาเท่าเดิมทุกช่อง) ⇒ **deploy ไม่ต้องทำขั้นนี้ซ้ำ** — รันคำสั่งข้างบนซ้ำได้ (ต้องได้ "ไม่มีอะไรต้องเขียน") (รายงานจะเหลือแค่ของที่ยังไม่มี) · ถ้า image ที่ deploy อยู่ยังไม่มี
`--extras-only` ให้รันด้วยโค้ดของ main ที่ mount เข้าไปแบบอ่านอย่างเดียว
(`docker compose run --rm --no-deps -v "$PWD:/src:ro" … --entrypoint sh app -c "คัด /src ไป /work แล้วรัน"`)

ตรวจ: หน้า "สมุดราคา" → เปิดชีต TS-01+TS-01-0 ต้องเห็นคอลัมน์ "ชนิดสายมาตรฐาน" เป็นช่องเลือก (สแตนเลสถักทุกแถว) ·
`*M8x1.25` `*M10x1.5` สีแดง · คอลัมน์ 1/4” (TS-01) และ M5 (TS-01-0) พื้นเหลือง มีป้าย "มาตรฐาน" · หน้าคำนวณราคา
`TSJ-01 4.8+2M` = 240 · `TSK-01(M6)4.8+2MT` = 340 · `TSK-01-0(M6)+2MT` = 370 บาท · รันซ้ำทั้งสองคำสั่งต้องได้ "ไม่มีอะไรต้องเขียน"

**สายเทปล่อนหุ้มชีลด์ (TS) 160 บาท/ม. + TS_-01-0 รับ C/TS** (เจ้าของสั่ง 2026-09-25) — ราคา 160 **ไม่มีในไฟล์ Excel ไหน**
(ถอดจากราคาขาย Odoo · `docs/pricing-code-ts-01.md` §5) ⇒ อยู่ในฐานเท่านั้น · **ฐานของ PMSV เขียนแล้ว 2026-09-25**
(การบันทึกครั้งที่ 6 เฉพาะราคาสาย TS ของ TSK-01 + TSK-01-0 · แถวรหัสย่อย C/TS ของ `TSK-01-0` 2 แถวจาก `catalog-subcodes.json`
· สำรองที่ `backup/pricing-before-ts-cable-2026-09-25-1008.dump`) ⇒ deploy ไม่ต้องทำซ้ำ · **ฐานใหม่:** รัน `seedCatalogSubcodes`
ข้างบน แล้วกรอก 160 ที่หน้าสมุดราคา → ชีต TS-01+TS-01-0 → แถบราคาสาย ช่อง "สายเทปล่อนหุ้มชีลด์" (ช่องนี้ขึ้นเองเมื่อมีแถวรหัสย่อย
ต้องใช้โค้ดตั้งแต่คอมมิตนี้) · ตรวจ: `TSK-01-0(M6)+2MTSU` = 350 · `TSP-01(M6)4.8+2MTSU` = 1,080 · `TSK-01(M6)4.8+2MC` = "ยังไม่มีราคาในสมุดราคา"

**ความยาวแกน `xNN` ของ TS_-01** (เจ้าของสั่ง 2026-09-25 — *"เริ่มคิดเงินเมื่อยาวเกิน std ของแต่ละรุ่น"* · *"ยึดราคาจาก excel"*) —
เกิน 5 mm (แคตตาล็อก) คิด 100 mm ละ ปัดขึ้น แกน 4.8 = 110 · 6 = 120 (แมป 19 อ่าน `TS-06!C27/C31`) · เล่มที่มีอยู่แล้วเติมด้วย:

```bash
npx tsx scripts/pricebook/importer.ts --data <โฟลเดอร์ Excel> --new-rules                 # รายงาน — ต้องเห็นแค่ TSK-01 + len_l1
npx tsx scripts/pricebook/importer.ts --data <โฟลเดอร์ Excel> --new-rules --apply --by admin
```

`--new-rules` เติมเฉพาะกฎที่แมปมีแต่รุ่นในฐานยังไม่มี (+ `standard` ของมิตินั้นถ้ายังไม่มี) **ไม่แก้กฎ/ราคาเดิมสักช่อง** (ราคา TS 160
ที่อยู่ในฐานเท่านั้นจึงไม่หาย — ต่างจาก `--replace-all`) · **ฐานของ PMSV เขียนแล้ว 2026-09-25** (การบันทึกครั้งที่ 7 เฉพาะ TSK-01 ·
...
**ท้ายรหัส TS_-08 / TS_-10 ตามแคตตาล็อก** (แคตตาล็อกชุดเพิ่ม `backup/Catalogue/` 2026-09-25 · เจ้าของสั่ง "เขียนได้" 2026-09-25 ·
**ฐานของ PMSV เขียนแล้ว 2026-09-25** — 36 แถวรหัสย่อย + การบันทึกครั้งที่ 8–10 · สำรองที่ `backup/pricing-before-ts0810-2026-09-25-1346.dump`
⇒ deploy ไม่ต้องทำซ้ำ · **ต้องใช้โค้ดตั้งแต่คอมมิตนี้** — โค้ดเก่าอ่านแถว `option` และแถวค่าว่างไม่ออกแล้วข้ามทิ้งเงียบ ๆ = กลับเป็นท่อนแดงเหมือนเดิม ไม่ได้คิดผิด) —
TS_-08 `-[2]-[หัวกระโหลก][Ground]` เช่น `-2-KBU` · TS_-10 หลัง M = `[ชนิดสาย][Ground]` เช่น `+3MTU` ⇒ แถวใน `catalog-subcodes.json`:
หัว B/K/KB + 2 element เป็นรหัสย่อยแบบ **"เปิดกฎบวกเพิ่มของรุ่น"** (ราคาอยู่ในกฎเดิมของชีต R9–R11/U ไม่ได้พิมพ์ตัวเลขใหม่) ·
**หัว S/E/SS/SB + เกลียวมิลทุกขนาดที่มีในรหัสจริง เป็นแถว "ค่าว่าง"** (เจ้าของสั่ง "ใส่ค่าว่างไว้ก่อนค่อยกำหนดภายหลังผ่าน ui" — อ่านออก
แต่ขึ้น "ยังไม่มีราคา" จนกว่าแอดมินกรอกเงิน/เลือกคอลัมน์เกลียวที่หน้าสมุดราคา → ตารางรหัสย่อย → แก้) + ค่ามาตรฐานสาย TS-10 = พีวีซี
(แมป 16 · แคตตาล็อก "Standard for RTD" = `TS-10!A41`) + **ค่าสายเกินมาตรฐานนับเฉพาะเมตรเต็มทุกรุ่น** (ปัดลง — เจ้าของสั่ง: *"ยาวกว่าสาย std
เพิ่มขึ้นตั้งแต่ 1 M ขึ้นไป"* · แมปทั้ง 7 รุ่นที่มีกฎ `cable_over_1m`) + ราคาสาย TS ของ TSP-10 = 160 (เท่า TS_-01 · **อยู่ในฐานเท่านั้น ไม่มีใน Excel**)
· ฐานใหม่ทำตามลำดับ (สำรองสี่ตาราง `pricing_*` ก่อน · ไม่ใส่ `--apply` = รายงานอย่างเดียว):
```bash
npx tsx scripts/pricebook/seedCatalogSubcodes.ts --apply --by admin                                 # แถวที่ยังไม่มี (ครั้งนี้ + 36 แถว)
npx tsx scripts/pricebook/importer.ts --data <โฟลเดอร์ Excel> --extras-only --apply --by admin      # TSP-10: cable = สายพีวีซี
npx tsx scripts/pricebook/importer.ts --data <โฟลเดอร์ Excel> --rounding --apply --by admin         # 7 รุ่น: cable_over_1m ปัดขึ้น → ปัดลง
```
แล้วกรอก 160 ที่หน้าสมุดราคา → รุ่น TSP-10 → กฎ "ราคาสายส่วนที่ยาวเกิน 1 เมตร" ช่อง "สายเทปล่อนหุ้มชีลด์" (ช่องขึ้นเองเมื่อมีแถวรหัสย่อย TS)
· ตรวจที่หน้าคำนวณราคา: `TSP-08(S4)6x100-BU` = 2,180 · `TSP-08(S6)8x43-2-KBU` = 3,210 · `TSP-10(S4)6x100+3MTU` = 1,940 ·
`TSP-10(S2)5x55+5MTSU` = 2,125 · `TSP-10(S2)6x100+1.5MPU` = 1,535 (เกินไม่ถึง 1 M ไม่คิด) · `+2.5MPU` = 1,635 · `TSP-08(S4)6x100-SU` และ
`TSP-08(M8)6x100-BU` = "ยังไม่มีราคา" · รันซ้ำทุกคำสั่งต้องได้ "ไม่มีอะไรต้องเขียน"
...
### ขั้น 5 — rebuild + up
```bash
docker compose build app              # ช่วงที่นานที่สุด — ตัวเก่ายังตอบ LINE อยู่ตลอด · build ล้ม = ไม่มีอะไรถูกแตะ
docker compose up -d --no-build app   # ⏱ บอทเงียบเฉพาะบรรทัดนี้ — จดเวลาไว้ใช้ในขั้น 6
...
```
- **ทำไมแยก `build` ออกจาก `up`** — `up -d --build` ก็ build ก่อนสลับเหมือนกัน แต่แยกแล้วเราเลือก
  *จังหวะ* สลับได้เอง (build เสร็จตอนพีคก็รอให้ถึงช่วงเงียบก่อนค่อยสั่ง `up`) และ build ที่ล้ม
  จะไม่ไปถึงขั้นหยุดกล่องเลย
- **ระบุชื่อ `app` เสมอ** — `up -d` แบบไม่ระบุก็ไม่แตะ `tunnel` ถ้านิยามไม่เปลี่ยน แต่ระบุไว้กันพลาด
  (tunnel หลุด = โดเมนหายทั้งเส้น ดูหัวข้อ "ทางเข้าสาธารณะ")
...
**6.1 ทางเข้าจริงยังตอบ** — ต้องผ่าน public URL ไม่ใช่แค่ `127.0.0.1` และ **ต้องมี `-4`**
(เครื่องนี้ออก IPv6 ไม่ได้ ไม่ใส่จะค้างแล้วสรุปผิดว่าล่ม) · ห้ามยิง `POST /callback` เปล่า ๆ ทดสอบ
(ได้ 500 ลายเซ็นไม่ผ่าน ทิ้งแถวค้างใน `api_logs`)
```bash
curl -4 -sI https://salechatbot.primus-iot.com/ | head -1
```

**6.2 ข้อความที่ตกช่วงสลับกล่อง — ตรวจหลังสลับ 5–10 นาที** (LINE ส่งซ้ำมาช้าได้ถึง 4 นาทีครึ่ง)
```bash
docker compose logs -t --since 15m app | grep 'LINE ส่งซ้ำ'
```
⚠️ **ห้าม grep คำว่า `isRedelivery` เฉย ๆ** — log พิมพ์ body ของทุก webhook ซึ่งมี `"isRedelivery": false`
อยู่ทุกตัว (วัด 2026-09-23: grep แบบนั้นได้ 110 บรรทัดใน 24 ชม. ตัวจริงมี 4)
แต่ละบรรทัดคือข้อความของเซลส์ 1 คน (`user=U…`) — **เวลาใน log ลบด้วย "เหตุการณ์เดิมเมื่อ N วิที่แล้ว"**
= เวลาที่เซลส์ส่งจริง (เวลาจาก `-t` เป็น UTC — บวก 7 ชม.) ถ้าตกช่วงที่จดไว้ในขั้น 5 แปลว่า
**เขาไม่ได้คำตอบ** ให้ทักเขาให้ส่งใหม่ · ไม่มีบรรทัดไหนเลย = ไม่มีใครตกหล่น
บรรทัดที่ย้อนไปไม่ตรงช่วงนั้นคืออาการปกติที่ Cloudflare ตัด connection เอง (~4–5 ครั้ง/วัน ดู
`CLAUDE.md`) — กรณีนั้นเซลส์ได้คำตอบไปแล้ว ไม่ต้องทัก

**6.3 diag** ที่ **อ่านอย่างเดียว** รันได้บน prod:
```bash
...
## สำรองฐานข้อมูลอัตโนมัติ (autobackup)

ตั้งแต่ 2026-09-15 เครื่องนี้ดัมป์ฐานเองทุกวัน **ตี 3** เก็บไฟล์ล่าสุด **7 ชุด** ไว้ใน `backup/`
ตัวจับเวลาคือ cron ของผู้ใช้ `app_sales` ไม่ใช่ service ในกล่อง — backup จึงยังเดินแม้ container
`app` ล่ม (สิ่งเดียวที่มันพึ่งคือ docker กับ container `db`)

```bash
bash scripts/backup/installCron.sh          # ติดตั้ง/อัปเดต (รันซ้ำได้ ไม่ทับบรรทัดของโปรเจคอื่น)
bash scripts/backup/installCron.sh 04:30    # เปลี่ยนเวลา
bash scripts/backup/installCron.sh --remove # ถอดออก (ไฟล์เดิมไม่ถูกลบ)

npm run backup:auto                # สั่งดัมป์เดี๋ยวนี้ (~18 วิ · ไฟล์ ~72 MB วัด 2026-09-15)
npm run diag:backup                # ตรวจทั้งสาย: cron · ความสดของไฟล์ · retention · ดิสก์ · log
npm run diag:backup -- --deep      # + เปิด TOC ของไฟล์ล่าสุดจริงว่ากู้ได้ (+~2 วิ)
```

**ดูสถานะจากหน้าแอดมินได้** — เมนู **รายงานการใช้งาน → แท็บ "การสำรองข้อมูล"** แสดงสถานะล่าสุด
จำนวนชุดที่เก็บ ที่ว่างบนดิสก์ และประวัติทุกรอบ (รวมรอบที่ล้มและเหตุผล) · ข้อมูลมาจากตาราง
`backup_runs` ที่สคริปต์เขียนเองตอนจบทุกรอบ ⇒ **ต้องรัน migration
`2026-09-15_03_backup_runs.sql` ก่อน** ไม่งั้นหน้านั้นจะขึ้นว่า "ยังไม่ได้เปิดระบบสำรองอัตโนมัติ"
(หน้าไม่พัง แต่ไม่มีข้อมูล) · การเขียนรายงานล้มเหลวไม่เคยทำให้การสำรองล้ม — เสียแค่แถวในรายงาน
· หน้านี้ตั้งใจไม่มีปุ่มดาวน์โหลดไฟล์ · ปุ่มกู้คืน · ปุ่มสำรองเดี๋ยวนี้ (เหตุผลอยู่หัวไฟล์
`frontend/src/admin/logs/BackupReport.tsx`)

**ลำดับที่ทำให้ "ไม่มีไฟล์ ดีกว่าไฟล์ที่กู้ไม่ได้"** — ดัมป์ลง `/tmp` ในกล่อง → `pg_restore --list`
อ่าน TOC พิสูจน์ว่าไฟล์ไม่ถูกตัด → copy ออกมาเป็น `.part` บน host → ผ่านด่านขนาดแล้วจึง `mv`
เป็นชื่อจริง → **ลบของเก่าเป็นขั้นสุดท้าย** ⇒ รอบที่ล้มไม่เคยทำให้ของเก่าหายแม้แต่ไฟล์เดียว

* ลบเฉพาะไฟล์ที่ขึ้นต้น `auto_` — dump ที่ทำเองด้วยมือไม่ถูกแตะ
* retention **นับไฟล์ ไม่ใช่นับวัน**: cron หยุดไปหลายวันก็ยังเหลือ 7 ชุดล่าสุดเสมอ
* ดิสก์ว่างต่ำกว่า 1 GB = ไม่ดัมป์ (ดิสก์เต็มลาก postgres ของจริงล่ม แย่กว่าพลาด backup 1 วัน)
* log อยู่ที่ `backup/autobackup.log` (หมุนเองที่ 2 MB) · ความล้มเหลวก่อนสคริปต์เริ่มอยู่ที่
  `backup/autobackup.cron.log`
* เวลา 03:00 เลือกเพราะไม่ทับหน้าต่าง auto-sync (จ.–ศ. 07:10–17:50) — pg_dump ถือ
  AccessShareLock ตลอดการดัมป์ ถ้าชนกับ build+swap ของ `customers_data_view` (ต้องการ
  AccessExclusiveLock) รอบ sync จะรอจนดูเหมือนค้าง

**กู้คืนจากไฟล์อัตโนมัติ** — เหมือน dump ปกติทุกอย่าง:
...
พัง" เท่านั้น · ไฟล์มี PII ลูกค้า + bcrypt hash ⇒ ถ้าวันหนึ่งจะส่งออกนอกเครื่อง ต้องเข้ารหัสก่อน
